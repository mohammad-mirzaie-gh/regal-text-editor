...
- **Headless-first.** `@rte/core` has no DOM and no React dependency at all — it can run in Node for server-side document manipulation, validation, or format conversion, with no browser required.
...
```
Layer 1  @rte/core            Document model, schema, selection, transactions, transforms,
...
Layer 2  @rte/browser          DOM adapter: renders the model to HTML, maps DOM Selection <-> model
                               Point, handles keyboard / beforeinput / IME composition / clipboard /
                               drag-drop. No React dependency.

Layer 3  @rte/ui                Pre-built toolbar UI (<Toolbar>, <BubbleToolbar>, <LinkButton>,
                               <ImageButton>, <FindReplacePanel>, dropdowns). Built only on
                               @rte/react's public hooks — never reaches into engine internals.

Layer 4  plugin-*              Seven feature plugins (see §5), each independent and optional.

Layer 5  @rte/react            <RichTextEditor>, useEditor, useEditorSelector, useCommand.
                               The engine (@rte/core, @rte/browser) has no dependency on this layer.
```

A consumer can use the engine fully headless with no UI at all (`new Editor({...})`), pair the core with a custom DOM adapter instead of `@rte/browser`, or use the React binding with a completely custom toolbar instead of `@rte/ui`.
...
|---|---|---|---|---|
| `@rte/core` | Document model, schema, transactions, selection, commands, history, plugin system, HTML/Markdown/JSON/text serialization, HTML parsing + sanitization | — | — | 58.4 KB |
| `@rte/browser` | DOM rendering, DOM↔model selection mapping, keyboard/IME/clipboard/drag-drop handling | `@rte/core` | — | 22.8 KB |
| `@rte/react` | `<RichTextEditor>`, `useEditor`, `useEditorSelector`, `useCommand` | `@rte/core`, `@rte/browser` | `react` ≥18, `react-dom` ≥18 | 4.3 KB |
| `@rte/ui` | Default toolbar UI: `<Toolbar>`, `<BubbleToolbar>`, `<LinkButton>`, `<ImageButton>`, `<FindReplacePanel>`, dropdown items, icon set, theming CSS | `@rte/core`, `@rte/browser`, `@rte/react`, `@rte/plugin-link` | `react` ≥18, `react-dom` ≥18 | 19.1 KB JS + 13.0 KB CSS |
| `@rte/plugin-basic-marks` | Bold, italic, underline, strikethrough, inline code | `@rte/core` | — | 4.5 KB |
| `@rte/plugin-basic-blocks` | Paragraph, heading (configurable levels), blockquote, horizontal rule, text alignment | `@rte/core` | — | 9.4 KB |
| `@rte/plugin-lists` | Bullet / ordered / task lists, nesting, indent/outdent | `@rte/core` | — | 9.7 KB |
| `@rte/plugin-link` | Link mark: `setLink` / `unsetLink`, sanitized href | `@rte/core` | — | 5.5 KB |
| `@rte/plugin-image` | Block-level image node, `insertImage` | `@rte/core` | — | 2.5 KB |
| `@rte/plugin-code-block` | Preformatted code block, Enter-inserts-newline behavior | `@rte/core` | — | 2.5 KB |
| `@rte/plugin-history` | Undo/redo commands and keyboard shortcuts | `@rte/core` | — | 0.5 KB |
...
### `@rte/plugin-basic-blocks` — block types & alignment
...
### `@rte/plugin-lists` — bullet, ordered & task lists
...
### `@rte/plugin-code-block` — preformatted code
...
`findMatches` / `replaceMatch` / `replaceAllMatches` live directly in `@rte/core` (case-insensitive by default, never matches across a block boundary even when adjacent blocks' text would otherwise read as contiguous), paired with `<FindReplacePanel>` in `@rte/ui`. Only the current match is highlighted via the real editor selection — there's no separate "highlight every match at once" decoration layer yet.

---

## 6. `@rte/ui` component reference
...
| `<LinkButton />` | Self-contained popover that collects a URL and calls `@rte/plugin-link`'s `setLink`/`unsetLink`; pre-fills the current link's href when editing an existing one. |
| `<ImageButton />` | Self-contained popover that collects an image URL + alt text and calls `@rte/plugin-image`'s `insertImage`. |
| `<FindReplacePanel />` | Find/replace UI driven by `@rte/core`'s search functions; Enter/Shift+Enter step forward/backward, Escape closes. |
...
**Theming**: import `@rte/ui/styles.css` once; every visual token is a CSS custom property on `:root` (colors, radii, shadows, spacing). Two alternate presets ship out of the box, toggled by an attribute on any ancestor element:
...
- Every package has dedicated unit tests, not just indirect coverage through integration tests — including the browser adapter, the React hooks, and every `@rte/ui` component.
...
```bash
npm install @rte/core @rte/browser @rte/react @rte/ui \
  @rte/plugin-basic-blocks @rte/plugin-basic-marks @rte/plugin-lists @rte/plugin-history
```
...
- **React** ≥ 18 for `@rte/react` and `@rte/ui` (peer dependency — not bundled, so it's never duplicated in a consumer's bundle). `@rte/core` and `@rte/browser` have no React dependency at all.
